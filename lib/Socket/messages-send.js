...
const makeMessagesSocket = B => {
    ...
    const E = B.userDevicesCache || new _nodeCache.default({
        ...
        useClones: false,
        maxKeys: B.maxDeviceCacheKeys || 500 // seraphbail: memory leak prevention
    });
    ...
    // seraphbail: auto-retry helper for relayMessage & sendMessage
    const _sRetry = async (fn, max, log) => {
        let lastErr;
        for (let i = 0; i < max; i++) {
            try {
                return await fn();
                ...
                lastErr = e;
                const fatal = e?.output?.statusCode === 401 || e?.output?.statusCode === 403;
                if (fatal || i >= max - 1) throw e;
                const delay = Math.min(1000 * Math.pow(2, i), 10000);
                log?.warn({ attempt: i + 1, code: e?.output?.statusCode }, "seraphbail: retrying send...");
    ...
    const sock = {
        ...
        assertSessions: Z,
        relayMessage: (jid, msg, opts) => _sRetry(() => ca(jid, msg, opts), B.maxMsgRetryCount ?? 3, m),
        sendReceipt: ra,
        sendReceipts: sa,
    ...
    // seraphbail: high-level Album message helper
    sock.sendAlbumMessage = async (jid, medias, options = {}) => {
        if (!Array.isArray(medias) || medias.length < 2) {
            throw new _boom.Boom("Album requires at least 2 media items");
        }
        const imageCount = medias.filter(x => x.image).length;
        const videoCount = medias.filter(x => x.video).length;
        if (imageCount + videoCount !== medias.length) {
            throw new _boom.Boom("Each album item must contain either 'image' or 'video'");
        }
        ...
        for (const media of medias) {
            const content = { ...media, albumParentKey: albumMsg.key };
            const sent = await sock.sendMessage(jid, content, options);
            sentMedia.push(sent);
            // small natural delay between album items to avoid burst send pattern
            await new Promise(r => setTimeout(r, 300 + Math.floor(Math.random() * 400)));
        }
        return { album: albumMsg, media: sentMedia };
    };
    return sock;
};
